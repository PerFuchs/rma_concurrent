...
PackedMemoryArray::PackedMemoryArray(size_t btree_block_size, size_t pma_segment_size, size_t pages_per_extent, size_t num_worker_threads, size_t segments_per_lock) :
            ...
            m_vertex_index(new VertexIndex(ARGREF(size_t, "vertices"))),
 ...
 *****************************************************************************/
        ...
        if (check_fence_keys(gate, /* in/out */ gate_id, key)) {
          switch (gate.m_state) {
            ...
            case Gate::State::REBAL:
              ...
              gate.m_queue.append({Gate::State::WRITE, context});
              lock.unlock();
              context->wait();
          }
        }
    ...
    void PackedMemoryArray::writer_on_exit(Gate *gate, bool is_insert, bool is_delete, bool rebalance) {
      assert(gate != nullptr);
      ...
      if (is_insert)
        gate->m_cardinality++; // we've just inserted a new element
      else if (is_delete)
        gate->m_cardinality--; // this was a deletion
 ...
 *****************************************************************************/
    ...
    const data_structures::rma::common::CachedDensityBounds &PackedMemoryArray::get_thresholds() const {
    ...
    void PackedMemoryArray::set_thresholds(const RebalancePlan &action) {
      if (action.m_operation == RebalanceOperation::RESIZE ||
          action.m_operation == RebalanceOperation::RESIZE_REBALANCE) {
    ...
    rma::common::Knobs &PackedMemoryArray::knobs() {
      return m_knobs;
    }
    ...
    void PackedMemoryArray::insert(int64_t key, int64_t value) {
      ...
      do {
        try {
          ...
          Gate *gate = insert_on_entry(key); // lock the gate where we're going to insert the new element
...
//            adjust_vertex_index_on_insert(vertex_index, segment_id, key);
    ...
    Gate *PackedMemoryArray::insert_on_entry(int64_t key) {
      return writer_on_entry(key);
    }
    ...
    bool PackedMemoryArray::insert_common(size_t segment_id, int64_t key, int64_t value) {
      ...
      assert(segment_id < m_storage.m_number_segments &&
             "Overflow: attempting to access an invalid segment in the PMA");
 ...
 *****************************************************************************/
    int64_t PackedMemoryArray::remove(int64_t key) {
      ...
      do {
        try {
          ...
          Gate *gate = remove_on_entry(key); // lock the gate where we're going to insert the new element
          assert(gate != nullptr && "Null gate");

          VertexIndex *vertex_index = m_vertex_index.get(get_context());

          bool need_global_rebalance = do_remove(gate, key, &value);

          if (value != -1) {
            adjust_vertex_index_on_remove(vertex_index, key);
          }
          remove_on_exit(gate, /* successful ?*/ value != -1, need_global_rebalance);
          done = true;
        } catch (Abort) {}
      } while (!done);
    ...
    Gate *PackedMemoryArray::remove_on_entry(int64_t key) {
      return writer_on_entry(key);
    }

    void PackedMemoryArray::remove_on_exit(Gate *lock, bool successful, bool rebalance) {
      writer_on_exit(lock, /* insertion ? */ false, /* deletion ? */ successful, /* rebalance ? */ rebalance);
    }
 ...
 *                                                                           *
 *****************************************************************************/
    void PackedMemoryArray::rebalance_global(Gate *gate) {
      assert(gate != nullptr && "Null pointer");
      ...
      gate->m_num_active_threads = 0;
      gate->m_queue.prepend({Gate::State::WRITE, get_context()});
 ...
 *****************************************************************************/
    ...
    bool is_local_rebalance = rebalance_find_window(segment_id, is_insert, &window_start, &window_length, &cardinality, &do_resize);
      ...
      adjust_vertex_index_for_chunk(metadata.m_window_start, metadata.m_window_length);
...
RebalancePlan PackedMemoryArray::rebalance_plan(bool is_insert, int64_t window_start, int64_t window_length, int64_t cardinality_after, bool resize) const {
    COUT_DEBUG("is_insert: " << is_insert << ", window_start: " << window_start << ", window_length: " << window_length << ", cardinality_after: " << cardinality_after << ", resize: " << resize);
      ...
      RebalancePlan result{const_cast<CachedMemoryPool &>( m_memory_pool )};
 ...
 *****************************************************************************/
    ...
    /**
     * Finds position of a key within a segment.
     *
     * @param segment_id
     * @param key
     * @return
 ...
 *                                                                           *
 *****************************************************************************/
    size_t PackedMemoryArray::get_segment_id(size_t index) {
      return index / get_segment_capacity();
    }

/**
 * Resets all vertex index positions for the segments between window_start and window_start + window_length.
 *
 * We adjust the index per segment. To know if we should reset also the fist vertex in a segment, we need to know
 * if it starts in this segment. For all segments but the first, we get this knowledge for free because we just reset
 * these segments before.
 * However, for the first segment in the window, we don't know the last vertex from the segment before and we cannot check
 * as that segment is not locked by us. However, we know that all operations (resize, rebalance, insert, delete) calling
 * this change only vertices within the window. Hence, if the first vertex of the first segment started within the window
 * before the operation, it will do after. Then we reset it. If it's been outside of the window before the operation, it will
 * start outside after and we do not reset it.
 *
 * @param window_start
 * @param window_length
 */
...
//      return;
      ...
      if (window_start % 2 == 0) {
        first_entry_pos = m_storage.m_segment_capacity - m_storage.m_segment_sizes[window_start];
      }

      uint64_t first_vertex = TO_SRC(m_storage.m_keys[window_start * m_storage.m_segment_capacity + first_entry_pos]);
      auto first_vertex_segment_id = get_segment_id(vertex_index->get_vertex_start(first_vertex));
      // If the first vertex started in the window before the operation, it will do after. Therefore, reset it's vertex position.
      if (window_start <= first_vertex_segment_id && first_vertex_segment_id < window_start + window_length) {
        first_vertex = VertexIndex::UNDEFINED;
      }

      uint32_t vertex_before_segment = first_vertex;
      for (int i = window_start; i < window_start + window_length; i++) {
        vertex_before_segment = adjust_vertex_index_for_segment(i, vertex_before_segment, vertex_index);
      }
    }

    uint32_t PackedMemoryArray::adjust_vertex_index_for_segment(size_t segment_id, uint32_t last_vertex, VertexIndex* vertex_index) {
      const auto segment_start = segment_id * m_storage.m_segment_capacity;
      size_t segmentSize = m_storage.m_segment_sizes[segment_id];

      int64_t *__restrict keys = m_storage.m_keys + segment_start;

      size_t start = 0;
      size_t stop = segmentSize;
      if (segment_id % 2 == 0) { // for even segment ids (0, 2, ...), scan size to end
        start = m_storage.m_segment_capacity - segmentSize;
        stop = m_storage.m_segment_capacity;
      }

      for (size_t i = start; i < stop; i++) {
        if (TO_SRC(keys[i]) != last_vertex) {
          last_vertex = TO_SRC(keys[i]);
          vertex_index->set_vertex_start(last_vertex, segment_start + i);
        }
      }
    ...
    VertexIndex *PackedMemoryArray::get_vertex_index() const {
      return m_vertex_index.get(get_context());
    }
    ...
    void PackedMemoryArray::adjust_vertex_index_on_remove(VertexIndex *vertex_index, int64_t key) {
      auto src = TO_SRC(key);
      adjust_vertex_index_for_chunk(get_segment_id(vertex_index->get_vertex_start(src)), 1);

      if (TO_SRC(m_storage.m_keys[vertex_index->get_vertex_start(src)]) != src) {
        vertex_index->set_undefined(src);
      }
    }
